...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"data" | "settings" | "print">(
    "data",
  );
  ...
  return (
    <>
        ...
        <header className="bg-white border-b border-[#1A1A1A] px-8 py-4 sticky top-0 z-20 print:hidden flex justify-between items-center shrink-0">
          <div className="flex items-baseline gap-4">
            <h1 className="text-2xl font-bold tracking-tight uppercase">
              Sistem Manajemen Raport
            </h1>
            <span className="text-xs font-sans tracking-widest text-gray-500 uppercase hidden sm:block">
              Edisi Profesional v2.4
            </span>
          </div>
              ...
              <div className="print:hidden bg-white p-6 border-b border-[#1A1A1A] flex items-center justify-between shrink-0">
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-3">
                    <label className="text-xs font-sans font-bold uppercase tracking-widest">
                      Pilih Siswa:
                    </label>
                    <select
                    ...
                    >
                      {students.map((s) => (
                        <option key={s.id} value={s.id}>
                          {s.urut}. {s.nisn} - {s.nama || "(Tanpa Nama)"}
                        </option>
                      ))}
                    </select>
                  </div>
                  <div className="flex items-center gap-3 border-l border-[#1A1A1A] pl-6">
                    ...
                    <select
                      value={paperSize}
                      onChange={(e) =>
                        setPaperSize(e.target.value as "A4" | "F4")
                      }
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <span className="text-[9px] text-[#1A1A1A] max-w-[200px] text-right">
                    *Gunakan fitur "Simpan sebagai PDF" pada jendela Print.
                    (Buka Tab Baru/Deploy jika macet)
                  </span>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-auto bg-[#E5E7EB] p-10 flex justify-center print:hidden print:p-0">
                <div className="shadow-2xl transition-transform transform origin-top w-max bg-white/5 pb-10">
                  {selectedStudent ? (
                    ...
                    <div
                      className={`w-[210mm] ${paperSize === "F4" ? "h-[330mm]" : "h-[297mm]"} bg-white flex items-center justify-center text-gray-400 font-sans tracking-widest text-sm border-2 border-[#1A1A1A]`}
                    >
        ...
        <footer className="bg-[#1A1A1A] text-white px-8 py-2 text-[10px] font-sans flex justify-between items-center shrink-0 print:hidden">
          <div>
            ...
            <span className="text-green-400">
              TERHUBUNG KE DATABASE PENDIDIKAN
            </span>
          </div>
      ...
      <PrintGuide
        ...
        onConfirm={proceedToPrint}
        onCancel={() => setShowPrintGuide(false)}
        paperSize={paperSize}
      />
    </>
  );
